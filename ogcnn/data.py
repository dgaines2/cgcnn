...
def collate_pool(dataset_list):  # batch the crystal atoms
    ...
    batch_cif_ids = []
    base_idx = 0
    for i, (([atom_fea, hot_fea], nbr_fea, nbr_fea_idx), target, cif_id) in enumerate(
        dataset_list
    ):
    ...
    return (
        (
            [torch.cat(batch_atom_fea, dim=0), torch.cat(batch_hot_fea, dim=0)],
...
class CIFData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for key in elements.keys():
            ...
            hvs[element] = hv
        hot_fea = np.vstack(
            [make_hot_for_atom_i(crystal, i, hvs) for i in range(len(crystal))]
        )
